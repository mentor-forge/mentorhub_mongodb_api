...
- [x] Created Mentee.0.1.0.0.json (7 documents, no `mentor_id`)
- [x] Configure database SUCCESS verified
...
- Edited **Profile.0.1.0.yaml**: added `full_name` (`sentence`), removed `schedule`.
- Added **Mentee.0.1.0.yaml** and **Mentee.yaml** (no `mentor_id`; unique index on `profile_id`).
- Updated **Profile.0.1.0.0.json**: `full_name` on all 15 profiles; `schedule` removed; `mentor_id` retained on mentee profiles.
- Created **Mentee.0.1.0.0.json**: 7 shared dossier records keyed by `profile_id`.

**Testing results**

- `DELETE /api/database/` → SUCCESS
- `POST /api/configurations/` → SUCCESS (Profile and Mentee 0.1.0.0)
- MongoDB: 15 Profiles with `full_name`, 0 Profiles with `schedule`, 7 Mentees, 0 Mentees with `mentor_id`
